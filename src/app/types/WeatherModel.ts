// Entityは主にinterface（type/型）、型ガードが記載されているレイヤー
// To parse this data:
//
//   import { Convert, WeatherModel } from "./file";
//
//   const weatherModel = Convert.toWeatherModel(json);
//
...
export class Convert {
  public static toWeatherModel(json: string): WeatherModel {
    return cast(JSON.parse(json), r('WeatherModel'));
  }

  public static weatherModelToJson(value: WeatherModel): string {
    return JSON.stringify(uncast(value, r('WeatherModel')), null, 2);
  }
}
...
const typeMap: any = {
  ...
  Clouds: o([{ json: 'all', js: 'all', typ: 0 }], false),
  Coord: o(
    [
